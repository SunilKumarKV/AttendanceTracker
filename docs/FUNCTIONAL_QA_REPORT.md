...
| Student import | Fixed | `xlsx` dependency had high-severity advisories with no fix. | Vulnerable SheetJS package used for Excel parsing. | `package.json`, `pnpm-lock.yaml`, `src/components/Students.tsx` | Replaced with `read-excel-file/browser`; audit now reports zero production vulnerabilities; build/lint pass. |
...
| Security checks | Fixed | Production audit reported `xlsx` vulnerability. | Vulnerable frontend dependency. | `package.json`, `pnpm-lock.yaml`, `src/components/Students.tsx` | `pnpm audit --omit=dev` passes for frontend and backend. |
...
cd backend && pnpm prisma migrate dev --name qa_verification --skip-seed
cd backend && pnpm run prisma:seed
...
pnpm audit --omit=dev
cd backend && pnpm audit --omit=dev
```
