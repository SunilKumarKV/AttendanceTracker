...
test.beforeEach(async ({ page }) => {
  ...
  await page.route(api('/attendance/sessions**'), async (route) => {
    if (route.request().method() === 'POST') {
      await route.fulfill({ status: 201, json: { success: true, data: { id: 'session-1', isLocked: false, records: [] } } });
      return;
    }
  ...
  await page.route(api('/classes**'), async (route) => route.fulfill({ json: { success: true, data: { items: [], pagination: { page: 1, pageSize: 100, total: 0, totalPages: 1 } } } }));
  await page.route(api('/subjects**'), async (route) => route.fulfill({ json: { success: true, data: { items: [], pagination: { page: 1, pageSize: 100, total: 0, totalPages: 1 } } } }));
  await page.route(api('/sections**'), async (route) => route.fulfill({ json: { success: true, data: { items: [], pagination: { page: 1, pageSize: 100, total: 0, totalPages: 1 } } } }));
