...
export const getDashboard = async (context: AdminContext) => {
  ...
  const [totalStudents, presentToday, absentToday, students, recentSessions, setupCounts, institution, v12Counts, staffCounts] =
    await Promise.all([
      ...
      Promise.all([
        ...
        prisma.staffAttendanceRecord.count({ where: { institutionId, attendanceDate: { gte: today, lt: tomorrow } } }),
        prisma.staffAttendanceRecord.count({ where: { institutionId, attendanceDate: { gte: today, lt: tomorrow }, status: 'ABSENT' } }),
        prisma.staffLeaveRequest.count({ where: { institutionId, status: 'PENDING' } }),
      ]),
    ]);
  ...
  return {
    ...
    staffSummary: {
      totalStaff: staffCounts[0],
      todayStaffAttendance: staffCounts[1],
      absentStaffToday: staffCounts[2],
      pendingStaffLeaves: staffCounts[3],
    },
    setupChecklist: {
...
export const createProfessor = async (context: AdminContext, data: any) => {
  const institutionId = requireInstitutionId(context.institutionId);
  await assertWithinPlanLimit(institutionId, 'teachers');
