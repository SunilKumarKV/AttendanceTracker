...
import { ArrowLeft, Building2, Eye, EyeOff, GraduationCap, Loader2, Lock, Mail } from 'lucide-react';
...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const user = await login(email, password, institutionCode);
      toast.success('Welcome back.');
      navigate(user.role === 'SUPER_ADMIN' ? '/platform' : user.role === 'ADMIN' || user.role === 'HOD' ? '/dashboard' : user.role === 'STUDENT' ? '/student-dashboard' : user.role === 'PARENT' ? '/parent-dashboard' : user.role === 'STAFF' ? '/staff-dashboard' : '/professor-dashboard');
    } catch (err) {
      ...
      <div className="mx-auto grid min-h-screen max-w-7xl grid-cols-1 lg:grid-cols-[1fr_0.92fr]">
        <section className="hidden flex-col justify-between bg-slate-950 p-10 text-white lg:flex">
          ...
          <div className="grid grid-cols-5 gap-4">
            {['Super Admin', 'Admin', 'Teacher', 'Student', 'Parent', 'Staff'].map((item) => (
        ...
        <main className="flex flex-col justify-center px-4 py-10 sm:px-8 lg:px-14">
          ...
          <div className="mx-auto w-full max-w-md">
            <div className="mb-8">
              ...
              <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">Use your admin, teacher, student, parent, or staff account to continue.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="rounded-3xl border border-slate-100 bg-white p-6 shadow-xl shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none sm:p-8">
              <div className="space-y-5">
                <div className="space-y-1.5">
                  <label htmlFor="institutionCode" className="text-sm font-bold text-slate-700 dark:text-slate-200">Institution Code</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-slate-200 py-3 pl-10 pr-4 font-medium uppercase outline-none transition-all focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:border-slate-700 dark:bg-slate-950"
                    />
                  </div>
                </div>
